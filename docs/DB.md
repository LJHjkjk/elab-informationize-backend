...
因为casdoor中有大量的估计用不到的字段（光user就有191个字段，头秃），且有储存额外信息的需求，可能会准备设计一些额外的表，使用外键关联到casdoor的user表。
...
系统可以发布群通知，也可以给对应的人发布邮件来让其处理事物。这个没有结果收集。
